...
import { Bell, Loader2, LogOut, Menu, Search, UserRound } from "lucide-react"
...
export function DashboardTopbar({
  ...
  const displayName = user.name || site.owner
  ...
  async function handleSignOut() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : labels.topbar.signOut)
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuLabel>
              ...
              <span className="block truncate text-xs font-normal text-muted-foreground">
                {user.role === "admin" ? labels.topbar.admin : "User"}
              </span>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem>
